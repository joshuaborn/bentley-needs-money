...
export default function ActionBar({modeState, setModeState}:ActionBarProps) {
    const handleNewExpenseClick = (event:SyntheticEvent): void => {
        ...
        setModeState({ mode: 'new expense' });
    }
    const handleNewPaybackClick = (event:SyntheticEvent): void => {
        event.preventDefault();
        setModeState({ mode: 'new payback' });
    }
    return (
        <div className="action-bar my-0">
            <div className="container buttons">
                <button className="button" disabled={modeState.mode === 'new expense'} onClick={handleNewExpenseClick}>
                    <span className="icon is-medium">
                        <i className="fa-solid fa-plus fa-lg" aria-hidden="true"></i>
                    </span>
                    <span className="text">New Expense</span>
                </button>
                <button className="button" disabled={modeState.mode === 'new payback'} onClick={handleNewPaybackClick}>
                    <span className="icon is-medium">
